...
class TestDashboardWorkflow:
    ...
    def test_area_gate_dropdown(self, driver):
        ...
        with allure.step("Verify and interact with Shutter-Gate checkbox"):
            ...
            shutter_gate_box = checkboxes[1]
            shutter_label = labels[1]

            initial_state = shutter_gate_box.is_selected()

            shutter_label.click()
            WebDriverWait(driver, 3).until(
                lambda d: shutter_gate_box.is_selected() != initial_state
            )

            assert shutter_gate_box.is_selected() != initial_state, \
